...
public class CheckUtils {
     ...
     * @Author Jce
     * @Description 身份证号校验
     * @Date 18:39 2019/12/25
     * @Param [iDNumber]
     **/
    public static boolean isIDNumber(String iDNumber) {
        if (iDNumber == null || "".equals(iDNumber) || (iDNumber.length() != 15 && iDNumber.length() != 18)) {
            return false;
        }
        //获取日期
        String d = iDNumber.substring(6, 14);
        boolean isDateStr = isDateStr(d);
        if (!isDateStr) {
            System.err.println("证件非法， 出生日期无效 ：" + d);
            return false;
        }
